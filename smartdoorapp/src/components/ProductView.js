...
function Productview() {
  ...
  async function productDetail() {
    try {
      const product = await axios.get(
        `https://smartdoor-fronted.onrender.com/productdetail/${productId}`
      );
    ...
    if (localStorage.getItem("token") == undefined) {
      ...
      try {
        const product = await axios.post(
          `https://smartdoor-fronted.onrender.com/addtocart`,
  ...
  async function buyNowFunction() {
    ...
    try {
      const buynowproduct = await axios.get(
        `https://smartdoor-fronted.onrender.com/buynowproduct/${productId}/${userId}`
      );
      ...
      {/*product add to the cart message =------------------------------  */}
      {
        ...
        >
          <p>
            Product_Add to cart
            <span>
              {product_add_number > 0 ? product_add_number : product_add_number}
            </span>
          </p>
        </div>
      }
      ...
      <div className="row pv-row ">
                  ...
                  ? product.img.map((img, index) => {
                      return (
                        <div className="indicator-img">
                          <button
                            type="button"
                            data-bs-target="#pvslider"
                            className=" active w-100 d-flex align-items-center justify-content-center"
        ...
        <div className="col-sm-7 col-12 pv-img-detail">
          ...
          <div className="cart-buy-button ">
            <div onClick={AddTocart} className="btn add-to-cart-btn">
              Add to cart
            </div>
