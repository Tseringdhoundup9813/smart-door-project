...
export default function BuyNow() {
    ...
    useEffect(()=>{
        ...
        async function placeOrder(){
            ...
            try{
                const payorder = await  axios.post(`http://localhost:3001/orders/${localStorage.getItem("user_id")}`,{address:address,number:contact})
    ...
    async function getDistrict(){
        ...
        try{
            ...
            const res = await axios.get(`https://nepallocation.com.np/api/v1/province/${province_id}/district`, {
                headers: {
    ...
    async function getProvince(){
       let token ="FF60UFp-JiGvK-hbrvrMNJe9"
       try{
            const res = await axios.get("https://nepallocation.com.np/api/v1/province/list", {
                headers: {
                    Authorization: "Bearer " + token,
    ...
    async function getCity(){
        let token ="FF60UFp-JiGvK-hbrvrMNJe9"
        try{
             const res = await axios.get(`https://nepallocation.com.np/api/v1/district/${district_id}/cities`, {
                 headers: {
                     Authorization: "Bearer " + token,
                ...
                <div className="col-md-7 col-lg-8 col-sm-7 col-xsm-12">
                    <div className="buy-deliver">
                        ...
                        <div className="b-d-address">
                            <div className="bd-address"> <span className="number"><input onChange={(e)=>setContact(e.target.value)}placeholder='enter your number'></input><br></br></span>


                            {/* select province ==================================== */}
                            <select  name="province" id="province"   onChange={setallprovincedata}>
                                { 
                                    province?
                                    
                                    province&&province.map((province,key)=>{
                                        return <option key={key} value={[province.name+"/"+province.province_id]} >{province.name}</option>
                                    })
                                    :<option value="select your">select your province</option>
                                }
                            ...
                            {/* select district =========================================== */}
                            <select  name="cars" id="cars"   onChange={setalldistrictdata}>
                                { 
                                    province_name?
                                    district&&district.map((item,key)=>{
                                        return <option key={key} value={[item.name+"/"+item.district_id]} >{item.name}</option>
                                    })
                            ...
                            {/* select  a city ================================== */}
                            <select  name="cars" id="cars"   onChange={(e)=>setcity_name(e.target.value)}>
                                { 
                                    district_name?
                                    city&&city.map((item,key)=>{
                    ...
                    <div className="buy-cart">
...

                        {
                            product&&product.map((product,key)=>{
                               return <div key={key} className="buy-cart-container">
                                    <div className="buy-cart-img" style={{backgroundImage:`url(${ product.img[0]})`}}></div>
                    ...
                    <div className="buy-subtotal">
                        ...
                        Subtotal : <span className="main-subtotal">Rs. {total_Amount}</span>
                    </div>
                    </div>
                </div>
                <div className="col-md-5 col-lg-4 col-sm-5 col-xsm-12">
                    <div className="buy-place-order">
                        ...
                        <div className="order-summary">
                            ...
                            <div className="items-total-amt text-end">Rs. {total_Amount}</div>
                        ...
                        <div className="placeorder-btn btn text-capitalize"onClick={placeOrder} ><NavLink to="/product/placeorder" className="nav-link" >place order</NavLink></div>
                    </div>
                </div>
            </div>
        </div>
